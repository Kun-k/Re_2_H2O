...
class Env:
    ...
    def step(self, action_ego, action_adv):
        ...
        new_state_ego = []
        if self.ego_policy == "uniform":
            new_state_ego = self.motion_model(self.get_state(0), [0, 0])
            traci.vehicle.moveToXY(vehID="car0",
                                   x=new_state_ego[0],
                                   y=new_state_ego[1],
                                   angle=-new_state_ego[3] * 180 / np.pi + 90,
                                   lane=0, edgeID=0)
            traci.vehicle.setSpeed(vehID='car0', speed=new_state_ego[2])
        elif self.ego_policy == "fvdm":
            self.ego_policy_model.run()
        elif self.ego_policy == "RL":
            new_state_ego = self.motion_model(self.get_state(0), action_ego)
            traci.vehicle.moveToXY(vehID="car0",
                                   x=new_state_ego[0],
                                   y=new_state_ego[1],
                                   angle=-new_state_ego[3] * 180 / np.pi + 90,
                                   lane=0, edgeID=0)
            traci.vehicle.setSpeed(vehID='car0', speed=new_state_ego[2])
        elif self.ego_policy == "genedata" or self.ego_policy == "realdata":
            new_state_ego = self.ego_policy_model[self.timestep]
            traci.vehicle.moveToXY(vehID="car0",
                                   x=new_state_ego[0],
                                   y=new_state_ego[1],
                                   angle=-new_state_ego[3] * 180 / np.pi + 90,
        ...
        if self.adv_policy == "uniform":
            for i in range(self.num_agents):
                new_state_adv.append(self.motion_model(self.get_state(i + 1), [0, 0]))
                traci.vehicle.moveToXY(vehID="car" + str(i + 1),
                                       x=new_state_adv[i][0],
                                       y=new_state_adv[i][1],
                                       angle=-new_state_adv[i][3] * 180 / np.pi + 90,
                                       lane=0, edgeID=0)
                traci.vehicle.setSpeed(vehID="car" + str(i + 1), speed=new_state_adv[i][2])
        elif self.adv_policy == "fvdm":
            for i in range(self.num_agents):
                self.adv_policy_model[i].run()
        elif self.adv_policy == "RL":
            for i in range(self.num_agents):
                new_state_adv.append(self.motion_model(self.get_state(i + 1), action_adv[2 * i: 2 * (i + 1)]))
                traci.vehicle.moveToXY(vehID="car" + str(i + 1),
                                       x=new_state_adv[i][0],
                                       y=new_state_adv[i][1],
                                       angle=-new_state_adv[i][3] * 180 / np.pi + 90,
                                       lane=0, edgeID=0)
                traci.vehicle.setSpeed(vehID="car" + str(i + 1), speed=new_state_adv[i][2])
        elif self.adv_policy == "genedata" or self.adv_policy == "realdata":
            for i in range(self.num_agents):
                new_state_adv.append(self.adv_policy_model[i][self.timestep])
                traci.vehicle.moveToXY(vehID="car" + str(i + 1),
                                       x=new_state_adv[i][0],
                                       y=new_state_adv[i][1],
                                       angle=-new_state_adv[i][3] * 180 / np.pi + 90,
        ...
        next_state = []
        if self.ego_policy == "sumo" or self.ego_policy == "fvdm":
            (x, y) = traci.vehicle.getPosition('car0')
            speed = traci.vehicle.getSpeed('car0')
            yaw = (90 - traci.vehicle.getAngle('car0')) * math.pi / 180
            new_state_ego = [x, y, speed, yaw]
        next_state.extend(new_state_ego)
        self.set_state(new_state_ego, 0)
        if new_state_ego[0] >= 240:
            self.ArrivedVehs.append(0)

        if self.adv_policy == "sumo" or self.adv_policy == "fvdm":
            for i in range(1, self.num_agents + 1):
                ...
                new_state_adv.append([x, y, speed, yaw])
        for i in range(1, self.num_agents + 1):
            next_state.extend(new_state_adv[i - 1])
            self.set_state(new_state_adv[i - 1], i)
            if new_state_adv[i - 1][0] >= 240:
                self.ArrivedVehs.append(i)
